...
<form id="rcp_registration_form" class="rcp_form" method="POST" action="<?php echo esc_url( rcp_get_current_url() ); ?>">
	...
	<fieldset class="rcp_user_fieldset">
		...
		<p id="rcp_password_wrap">
			...
			<div class="password-field">
				<input name="rcp_user_pass" id="rcp_password" class="required" type="text"/>
				<a href = "#" class="password-toggle" tabIndex="10">Hide</a>
			</div>
		</p>
		<p id="rcp_password_again_wrap">
			...
			<div class="password-field">
				<input name="rcp_user_pass_confirm" id="rcp_password_again" class="required" type="text"/>
				<a href = "#" class="password-toggle" tabIndex="10">Hide</a>
			</div>
		</p>
		<h2 style="margin: 1.25em 0 0; text-align: center;">Your Information</h2>
		<hr />
		...
		<ul id="rcp_subscription_levels">
			<?php foreach( $levels as $key => $level ) : ?>
				...
				<li id="rcp_subscription_level_<?php echo $level->id; ?>" class="rcp_subscription_level">
					<input type="radio" class="required rcp_level" <?php if( $key == 0 || ( isset( $_GET['level']) && $_GET['level'] == $key ) ){ echo 'checked="checked"';} if(isset( $_GET['id']) && $_GET['id'] == $level->id ){ echo 'checked="checked"';}?> name="rcp_level" rel="<?php echo esc_attr( $level->price ); ?>" value="<?php echo esc_attr( absint( $level->id ) ); ?>" <?php if( $level->duration == 0 ) { echo 'data-duration="forever"'; } ?>/>&nbsp;
	...
	<p id="rcp_submit_wrap">
		...
		<input type="submit" name="rcp_submit_registration" id="rcp_submit" value="<?php echo apply_filters ( 'rcp_registration_register_button', __( 'Register', 'rcp' ) ); ?>" disabled/>
	</p>
</form>
